...
nrSteps = 3  # -2 == undefined, -1 == unlimited
...
while flag:
    start = time.clock()
    system = readInputFile("input_files/np_w.pep")
   ...
   # a = 0.996
    ...
    for p_object in system.variables:
        ...
        if p_object.name == 'cruiseSpeedLeft':
             system.variables[system.variables.index(p_object)].value = 300
    ...
    # get speed value
    for p_object in system.variables:
        # if p_object.name == 'Err':
            # Err = p_object.value
        if p_object.name == 'Left':
            Left = p_object.value
        if p_object.name == 'Right':
            Right = p_object.value
